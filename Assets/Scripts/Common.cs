...
public static class Common
{
    ...
    private static byte[] GetImageByte(Bitmap bitmap)
    {
        byte[] bytes;
    ...
    /// <param name="ratioW">缩放宽的目标倍数</param>
    /// <param name="ratioH">缩放搞的目标的倍数</param>
    /// <param name="dstBmp">目标图像</param>
    /// <param name="zoomType">缩放选用的算法</param>
    /// <returns>处理成功 true 失败 false</returns>
    public static byte[] Zoom(Bitmap srcBmp, double ratioW, double ratioH,  Bitmap dstBmp, ZoomType zoomType)
    {
        byte[] bytes = null;
        //ZoomType为自定义的枚举类型
        if (srcBmp == null)
    ...
    public static byte[] Zoom(byte [] srcBmpBytes, double ratioW, double ratioH,  ZoomType zoomType)
    {
        MemoryStream ms = new MemoryStream(srcBmpBytes);
        Bitmap srcBmp = new Bitmap(ms);
        //ms.Dispose();
        ...
        unsafe
        {
            ...
            {//双线性插值法
                byte* srcPtrNext = null;
                int dstWidth = dstBmp.Width;
                int dstHeight = dstBmp.Height;
                ...
                for (int i = 0; i < dstHeight; i++)
                {
                    ...
                    for (int j = 0; j < dstWidth; j++)
                    {
                        srcdJ = j / ratioW;
                        srcJ = (int)srcdJ;//指向原始图像的列
                        if (srcdJ < 1 || srcdJ > srcWidth - 1 || srcdI < 1 || srcdI > srcHeight - 1)
                        {//避免溢出（也可使用循环延拓）
                            dstPtr[j * 3] = 255;
                            dstPtr[j * 3 + 1] = 255;
                            dstPtr[j * 3 + 2] = 255;
