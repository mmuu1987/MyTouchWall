...
public class MultiDepthMotion : InputMovementBase
{
    ...
    protected override void Init()
    {
        ...
        int levelCount = PictureHandle.Instance.showList.Count;

        //这里注意，posAndDir属于结构体，值拷贝
        for (int i = 0; i < datas.Length; i++)
        {
            PosAndDir data = datas[i];
            if (i < Common.PictureCount * levelCount)
            {
                temp.Add(data);
            }
            else
        ...
        _depthBuffer = new ComputeBuffer(levelCount, stride);
        _depths = new DepthInfo[levelCount];
    ...
    private void SetDepth(int top)
    {
        ...
        if (top >= PictureHandle.Instance.showList.Count) return;
